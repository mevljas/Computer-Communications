...
def client_thread(client_sock, client_addr):
    ...
    try:
        ...
        while loop:  # neskoncna zanka
            msg_received = receive_message(client_sock)
            if not msg_received:  # ce obstaja sporocilo
                break
            if ime is None:
                for client in allowed_clients:
                    if client == msg_received.upper():
                        ime = msg_received
                        print("[System] Uporabnik "+msg_received+" sprejet")
                        break
                else:
                    print("[System] Povezava zavrnjena za "+msg_received)
            else:
